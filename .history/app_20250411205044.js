...
for (let i = 1; i <= 15; i++) {
  ...
  li.addEventListener('click', () => {
    ...
    const existingSublist = dropdown.querySelector('.sublist');
    if (existingSublist) {
      existingSublist.classList.remove('open');
      setTimeout(() => dropdown.removeChild(existingSublist), 100); // Wait for the animation to finish
      if (existingSublist === li.querySelector('.sublist')) return; // Prevent toggling the same sublist
    }

    // Create a new sublist
    const sublist = document.createElement('ul');
    sublist.className = 'ml-5 mt-2 flex flex-col gap-3 sublist';

    for (let j = 1; j <= 8; j++) {
      const subItem = document.createElement('li');
      subItem.className = 'hover:text-(--primary-color) hover:font-bold';
      subItem.textContent = `Categoria`;
      sublist.appendChild(subItem);
    }

    dropdown.appendChild(sublist);
    setTimeout(() => sublist.classList.add('open'), 10); // Add the 'open' class after appending
  });
