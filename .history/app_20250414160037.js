...
for (let i = 1; i <= 15; i++) {
  ...
  li.addEventListener('click', () => {
    ...
    for (let j = 1; j <= 8; j++) {
      ...
      subItem.className = 'hover:text-(--primary-color) hover:font-bold first:text-(--primary-color) first:';
      subItem.textContent = `Categoria ${li.id}`;
      sublist.appendChild(subItem);
    }

    dropdown.appendChild(sublist);
    sublist.classList.add('open'); // Add the 'open' class after appending
  });
