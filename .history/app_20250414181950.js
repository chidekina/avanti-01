...
for (let i = 1; i <= 7; i++) {
  ...
  li.onclick = () => {
    ...
    const depId = `dep-${i}`;
    console.log(`Clicked on ${depId}`);

    // Example: Update content in the department-content-list
    depContentList.innerHTML = ''; // Clear existing content
    for (let j = 1; j <= 5; j++) {
      const subItem = document.createElement('li');
      subItem.textContent = `Subitem ${j} of ${depId}`;
      subItem.className = 'hover:text-(--primary-color) hover:font-bold';
      depContentList.appendChild(subItem);
    }
  };
