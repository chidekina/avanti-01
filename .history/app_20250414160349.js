...
for (let i = 1; i <= 15; i++) {
  ...
  li.addEventListener('click', () => {
    ...
    sublistContainer.className = 'flex gap-5 sublist-container';

    // Create 3 lists
    for (let listIndex = 1; listIndex <= 3; listIndex++) {
      const sublist = document.createElement('ul');
      sublist.className = 'flex flex-col gap-3 sublist';

      for (let j = 1; j <= 5; j++) {
        const subItem = document.createElement('li');
        subItem.className = 'hover:text-(--primary-color) hover:font-bold first:text-(--primary-color) f';
        subItem.textContent = `Categoria ${listIndex}-${j}`;
