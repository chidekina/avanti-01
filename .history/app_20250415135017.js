...
for (let i = 1; i <= 7; i++) {
  ...
  li.innerHTML = `
    <a href="#" id="dep-${i}" class="hover:font-bold hover:text-(--primary-color) department-menu"> Departamento </a>
  `;

  li.onclick = () => {


    const departmentMenu = depList.querySelectorAll('department-menu');
    departmentMenu.forEach(item => item.classList.remove('selected'));
