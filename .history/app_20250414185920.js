...
// Function to dynamically generate department content
function generateDepartmentContent(departmentId) {
  // Check if the content is already visible for the same department
  const currentVisibleId = depContent.getAttribute('data-visible-id');
  if (currentVisibleId === departmentId.toString()) {
    depContent.classList.add('hidden'); // Hide the content
    depContent.removeAttribute('data-visible-id'); // Clear the visible ID
    return;
  }

  // Update the visible ID and show the content
  ...
  for (let col = 1; col <= 3; col++) {
    ...
    for (let item = 1; item <= 8; item++) {
      ...
      li.textContent = `Categoria ${departmentId}-${col}-${item}`;
      ul.appendChild(li);
    }
...
for (let i = 1; i <= 7; i++) {
  ...
  li.onclick = () => {
    console.log(`Clicked on Departamento ${i}`);
    generateDepartmentContent(i); // Generate content dynamically for the clicked department
  };
